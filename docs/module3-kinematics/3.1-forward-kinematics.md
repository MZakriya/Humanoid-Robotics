...
def forward_kinematics(joint_angles, dh_params):
    ...
    for index, theta in enumerate(joint_angles):  # Loop variable index (changed from i to avoid MDX conflicts)
        a, alpha, d = dh_params[index]  # Index for dh_params
        T_index = dh_transform(a, alpha, d, theta + (0 if index < len(joint_angles) else 0))  # Transformation matrix T_index
        T_total = T_total @ T_index
